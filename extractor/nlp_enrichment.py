...
def _looks_like_company_name(value: str) -> bool:
    ...
    if any(re.search(pattern, normalized, re.IGNORECASE) for pattern in name_markers):
        return True

    if len(normalized.split()) <= 6 and not re.search(r"\b(?:objet|activit(?:[ée]s?)?|exploitation|commerce|vente|fabrication|production|service|services|etude|etudes|travaux|import|export|conseil|maintenance|agence|distribution|transport|tourisme|informatique|marketing|bureautique|industrie)\b", normalized, re.IGNORECASE):
        return True

    return False


def _looks_like_purpose(value: str) -> bool:
    normalized = _norm_text(value)
    if not normalized:
        return False
    ...
    return any(re.search(pattern, normalized, re.IGNORECASE) for pattern in purpose_markers)
    ...
    for idx, line in enumerate(lines):
        ...
        if result["corporate_purpose"] is None and CORE_LABEL_PATTERNS["corporate_purpose"].search(line):
            ...
            if candidate:
                ...
                if len(picked) >= 10 and _looks_like_purpose(picked) and not _looks_like_company_name(picked):
