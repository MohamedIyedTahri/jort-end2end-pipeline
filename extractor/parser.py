...
def _normalize_company_name(value: Optional[str]) -> Optional[str]:
    ...
    normalized = re.split(
        r"\s*(?:,|;|\.|\|)\s*(?:si[èe]ge|adresse|capital|objet|activit[ée]|dur[ée]e|g[ée]rance|g[ée]rant|administrateur|commissaire|matricule|quittance|d[ée]p[ôo]t)\b",
        normalized,
        maxsplit=1,
        flags=re.IGNORECASE,
    )[0].strip(" ,.;:-")
    ...
    if re.search(
        r"^(?:soci[ée]t[ée]\s+anonyme|soci[ée]t[ée]\s+[àa]\s+responsabilit[ée]\s+limit[ée]e|responsabilit[ée]\s+limit[ée]e|sarl|suarl|sa)$",
        normalized,
        re.IGNORECASE,
    ):
        return None
    ...
    if re.match(
        r"^(?:suivant|conform[ée]|constitution|en\s+vertu|selon|il\s+a\s+[ée]t[ée]|sous\s+le|au\s+capital|objet|activit[ée]|raison\s+sociale|d[ée]nomination)",
        normalized,
        re.IGNORECASE,
    ):
        return None
    ...
    if re.search(
        r"\b(acte|statuts?|enregistr[ée]|recette|finances?|greffe|tribunal|quittance|d[ée]p[ôo]t|matricule|num[ée]ro|n°|bureau|volume|folio|souscription|versement|constitu[ée]|constitu[ée]e|il\s+a\s+[ée]t[ée]\s+constitu[ée]|a\s+[ée]t[ée]\s+constitu[ée])\b",
        normalized,
        re.IGNORECASE,
    ):
        return None
    ...
    if re.search(
        r"\b(\d{1,2}/\d{1,2}/\d{2,4}|20\d{2}|19\d{2}|sous\s+le\s+n[°o]|sous\s+le\s+num[ée]ro|n[°o]\s*[A-Z0-9]|le\s+\d{1,2}/\d{1,2}/\d{2,4})\b",
        normalized,
        re.IGNORECASE,
    ):
        return None

    if not re.search(r"(?:^|\s)(?:[A-ZÀ-ÖØ-Ý][A-Za-zÀ-ÖØ-öø-ÿ'’\-]+|[A-ZÀ-ÖØ-Ý]{2,}|\d)", normalized):
        return None

    if len(re.split(r"\s+", normalized)) <= 1 and not re.search(r"[A-ZÀ-ÖØ-Ý]{2,}|\d", normalized):
        return None
...
def _normalize_address(value: Optional[str]) -> Optional[str]:
    ...
    normalized = re.split(
        r"\s*(?:,|;|\.|\|)\s*(?:matricule\s+fiscal|quittance|n°|num[ée]ro|suivant\s+acte|suivant\s+statuts?|il\s+a\s+[ée]t[ée]|d[ée]nomination|raison\s+sociale|capital|objet|activit[ée]|dur[ée]e|g[ée]rance|g[ée]rant|administrateur|commissaire|d[ée]p[ôo]t)\b",
        normalized,
        maxsplit=1,
        flags=re.IGNORECASE,
    )[0].strip(" ,.;:-")
...
def _normalize_capital_value(value: Optional[str]) -> Optional[str]:
    normalized = _normalize_text_value(value)
    if normalized is None:
        return None

    amount_match = re.search(
        r"(?P<amount>(?:\d[\d\s\.,]*)\s*(?:DT|D\.?T\.?|dinars?|dinar|dollars?|euros?))",
        normalized,
        re.IGNORECASE,
    )
    if amount_match:
        normalized = amount_match.group("amount")

    normalized = re.split(
        r"\s*(?:,|;|\.|\|)\s*(?:divis[ée]s?|souscrites?|lib[ée]r[ée]es?|dont|d[ée]claration|assembl[ée]e|conseil|d[ée]p[ôo]t|matricule|quittance|si[èe]ge|adresse|objet|activit[ée]|dur[ée]e|g[ée]rance|g[ée]rant)\b",
        normalized,
        maxsplit=1,
        flags=re.IGNORECASE,
    )[0].strip(" ,.;:-")
...
def _normalize_corporate_purpose(value: Optional[str]) -> Optional[str]:
    ...
    if re.search(r"\b(d[ée]nomination|raison\s+sociale|soci[ée]t[ée]|sarl|suarl|sa)\b", normalized, re.IGNORECASE):
        return None

    if len(re.split(r"\s+", normalized)) <= 6 and not re.search(
        r"\b(objet|activit(?:[ée]s?)?|exploitation|commerce|vente|fabrication|production|service|services|etude|etudes|travaux|import|export|conseil|maintenance|agence|distribution|transport|tourisme|informatique|marketing|bureautique|industrie|agricole|elevage)\b",
        normalized,
        re.IGNORECASE,
    ):
        return None
    ...
    for index, line in enumerate(lines[:40]):
        if re.search(r"Constitution|Cr[ée]ation|Notice", line, re.IGNORECASE):
            for candidate in lines[index + 1 : index + 8]:
                if ":" in candidate:
                    continue
                candidate_value = _normalize_company_name(candidate)
                if candidate_value is not None and 3 <= len(candidate_value) <= 180:
